...
from common.sshsrv import SSHHandler, command
from pdusim import PDUSim
import main
import password
    ...
    def command_ip(self, params):
        ...
        if params[0] == 'link':
            ...
            if params[0] == 'get':
                ...
                NetworkUtils.set_ip_address(ifname, params[2])
                if len(params) > 3:
                    netmask = params[3]
        ...
        if params[0] == 'start':
            ...
            if pdu_sim.is_alive():
                self.writeresponse("%svpdu service [%d] is alredy running.%s"
                                   % (colors.GREEN, pdu_sim.ident, colors.NORMAL))
            else:
                pdu_sim = PDUSim()
                pdu_sim.set_daemon()
                pdu_sim.start()
